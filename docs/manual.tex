...
\title{CAT User and Administrator Manual\\[2pt]\large A shared human and AI workspace for discussing a codebase}
...
commitment Slack does not: each channel is bound to a repository, and the
agents in that channel can read it.
...
\begin{itemize}
  ...
  \item \textbf{Permissions are enforced where they cannot be bypassed.}
...
The agents read and discuss. They cannot write files, push branches, or open
pull requests---there is no code path by which they could. Having an agent
propose real changes through a GitHub Action is a plausible next phase, but it
is a different trust decision and deserves to be made deliberately rather than
inherited.

There are also no threads, reactions, file uploads, or message editing.
Recreating Slack is not the goal; having somewhere to think about a codebase
together is.
...
\midrule
Paul & yes & yes & yes \\
Other members & yes & yes & no \\
Agents & yes & yes & --- \\
Anyone else & no & no & no \\
\bottomrule
\end{tabular}
\caption{The default arrangement. Permission to invoke agents is granted per
person in the \texttt{members} table.}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{signin.png}
...
Each channel corresponds to a repository, named for it, and the header links
directly to it on GitHub. Selecting a channel loads its history and subscribes
to live updates; the channel is reflected in the page URL, so a particular
channel can be linked to or bookmarked.
...
\begin{itemize}
  \item \textbf{Agents cannot change code.} Dispatching a GitHub Action that
    opens a pull request is the obvious next phase, and the schema already
    reserves \texttt{metadata} for recording the resulting branch or PR.
