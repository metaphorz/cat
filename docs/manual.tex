...
\texttt{/clear} & Saves the channel's transcript to your downloads, then
empties the channel on a second confirming command. See \S\ref{sec:clear}.\\
...
It always takes two commands. The first saves the transcript to your browser's
downloads as a Markdown file and deletes nothing at all. The second,
\texttt{/clear confirm}, does the deletion. The reason for the pause is that
deleted messages cannot be recovered from Supabase---row level security lets a
member delete only their own messages, so this runs with the service role, and
what it removes is gone.
