...
CAT is for reading a codebase together, not for changing one. An agent here
answers questions about the repository and has no way to write to it: no file
it can edit, no branch it can push. Ask one for a change and it will describe
the change concretely and say that a person has to make it.

Code is changed in a local agent session instead---in an editor such as
VS~Code, with the author watching the work as it happens and
redirecting it---and this workspace is where the result is reported
afterwards. The machinery
for asking an agent in a channel to open a pull request was built and still
exists, switched off since 22~September 2026; Section~\ref{sec:changes}
describes it and what re-enabling it would take.
...
role writes it. The path that fills it has been switched off since
22~September 2026 (Section~\ref{sec:changes}), so it is now a closed record
...
What you can ask for is an answer, not an edit. How something works, where a
discipline should look first, what a file is for, whether an approach is sound:
these are the normal case, and the agent has the repository in front of it.
A request to change the code is answered with a description of the change and
the observation that a person must make it---see Section~\ref{sec:changes}.
...
A link is not an attachment, but a link to a picture is still shown as one.
Any \texttt{http} or \texttt{https} URL in a message becomes a link, and if
its path ends in \texttt{.png}, \texttt{.jpg}, \texttt{.gif},
\texttt{.webp} or \texttt{.avif} the picture is drawn underneath it;
Markdown's own \texttt{!{[}alt{]}(url)} draws the picture alone, which is how
an agent writes one. The extension is the only evidence there is---asking the
far end what it holds would mean fetching it---so a link dressed as something
it is not simply fails to load. \texttt{.svg} is excluded for the reason
uploads exclude it.

Such an image stays where it lives. Nothing is copied into the bucket, so the
host serving it sees every reader of the message, the picture disappears the
day the link does, and the hour-long signing below does not apply to it. That
is the trade for being able to point at something already on the web rather
...
ten dollars against a few cents for a reply. Code changes are made in a local
agent session instead---an editor such as VS~Code, where the author watches the
work happen, redirects it when it goes wrong, and can run the tests against a
real environment---and land on the main branch from there. What follows
describes the mechanism as built, and remains accurate for anyone re-enabling
it.
...
\textsc{enter} completes the one you want. The two commands that are switches,
\texttt{/answer} and \texttt{/verbose}, carry their current position for this
channel next to the words they accept, so \texttt{on|off} says which of the
two you are already on rather than only what may be typed. The palette appears
only for members whose \texttt{is\_admin} flag is set.
...
than an essay, in this channel. Off by default; \texttt{on} asks for the full
treatment.\\
...
\begin{itemize}
  \item \textbf{The pull-request path ended at the pull request.} While it was
    on, an agent could open one but never saw CI results, reviewer comments, or
    whether the branch still merged cleanly a week later. Not seeing the work
    through is part of why the change is cheaper to make in an editor.
  \item \textbf{A writable channel makes the conversation an input to a
    process that writes code.} Everything typed in one may reach a job holding
    a write credential. With a single person permitted to request changes that
    was academic; it would stop being academic the moment a second person were
    granted the permission. All three switches are currently off.
